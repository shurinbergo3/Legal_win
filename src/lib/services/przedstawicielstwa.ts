...
const przedstawicielstwa: ServiceEntry = {
  ru: {
    ...
    faq: [
      { q: 'Чем oddział отличается от Sp. z o.o.?', a: 'Oddział — это структурное подразделение материнской фирмы, без отдельной правосубъектности. Все обязательства, активы, доходы — на материнской фирме. Sp. z o.o. — отдельное юрлицо с собственным капиталом и ответственностью. Oddział проще для холдинга, Sp. z o.o. — для нового бизнеса с польскими партнёрами.' },
      { q: 'Может ли przedstawicielstwo выставлять инвойсы?', a: 'Нет. Przedstawicielstwo вправе только: маркетинг, реклама, badania rynku, promocja, поиск партнёров. Продажи, услуги, производство — запрещены. Если нужно — закрываете przedstawicielstwo и открываете oddział или Sp. z o.o.' },
      { q: 'Какие налоги платит oddział?', a: 'CIT 19% на польский доход (przypisany do oddziału). Если есть umowa o unikaniu podwójnego opodatkowania — налог в Польше. VAT — оборотный налог 23%. Налоги материнской фирмы за рубежом — отдельно.' },
      { q: 'Сколько действует przedstawicielstwo?', a: '2 года с возможностью продления. Перед окончанием срока подаёте wniosek o przedłużenie за 30 дней. Если не продлили — przedstawicielstwo автоматически закрывается.' },
      { q: 'Нужен ли польский kierownik?', a: 'Нет. Kierownikiem oddziału или przedstawicielstwa может быть иностранец. Главное — у него должен быть PESEL и право legalnego pobytu w Polsce. Иначе невозможна регистрация в KRS и подача документов через ePUAP.' },
      { q: 'А если материнская фирма закрывается?', a: 'Oddział и przedstawicielstwo автоматически прекращают существование. Закрытие требует aktów и wniosków в KRS/MRiT — закрытие занимает 1–3 месяца с расчётом всех обязательств.' },
