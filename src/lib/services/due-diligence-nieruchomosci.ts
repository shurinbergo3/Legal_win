...
const dueDiligenceNieruchomosci: ServiceEntry = {
  ru: {
    ...
    faq: [
      { q: 'Зачем DD, если есть нотариус?', a: 'Нотариус проверяет KW и dokumenty при подписании, но не углубляется. Не запрашивает ZUS/US, не анализирует историю, не проверяет нелегальные пристройки, не смотрит plany miejscowe. Нотариус несёт ответственность только за to, что есть в aktach у него на столе. DD проверяет, что показала только prywatna агрегация. Видели сделки, где akt notarialny прошёл, а через год пришла hipoteka на 200к.' },
      { q: 'Можно ли отказаться от сделки после negativnogo DD?', a: 'Да, если в umowie przedwstępnej есть условие «wykonanie umowy uzależnione od pozytywnego due diligence prawnego». Если такого пункта нет — потеряете zaliczkę (5–10%). Поэтому пишем в каждой umowie przedwstępnej от наших клиентов. Если другая сторона не согласна — это уже red flag.' },
      { q: 'Какие самые частые «находки» DD?', a: 'Топ-5 находок за 2024: 1) Hipoteka не выплаченная или нерегулярно регулярная (30% случаев); 2) Нелегальные пристройки (рrzybudówka, веранда, дополнительный этаж — 25%); 3) Грaнicy не соответствуют ewidencji (15%); 4) Активные споры в суде (10%); 5) Долги podatków от nieruchomości (10%). Каждая может стоить 30 000+ PLN.' },
