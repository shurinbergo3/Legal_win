...
const tlumaczeniaPrzysiegle: ServiceEntry = {
  ru: {
    ...
    grounds: {
      ...
      items: [
        ...
        { title: 'Документы для TRC/ПМЖ', desc: 'Umowy o pracę, справки о доходах, выписки счетов, договоры аренды - пакет на иностранном переводим под ключ.' },
    ...
    faq: [
      ...
      { q: 'Сколько стоит перевод одного свидетельства?', a: 'Свидетельство о рождении/браке (1 страница) - 70-80 PLN. Диплом + приложение (3-5 страниц) - 250-400 PLN. Umowa o pracę (4-8 страниц) - 320-650 PLN. Точная цена - после оценки в течение часа.' },
  ...
  uk: {
    ...
    group: 'documents',
    icon: 'scale',
    badge: 'TEPIS · MS · 35+ мов',
    title: 'Присяжні переклади',
    subtitle: 'Tłumaczenia przysięgłe для польських urzędów, судів і банків',
    lead: 'Tłumacze przysięgli — це перекладачі зі спеціальною державною акредитацією Міністерства Юстиції, що мають право перекладати офіційні документи для польських urzędów, sądów, KRS, USC і банків. Без присяжного перекладу ваші іноземні документи не приймуть — навіть ідеально перекладені звичайним перекладачем. Працюємо з 35+ мовами: українська, російська, білоруська, англійська, німецька, іспанська, грузинська, вірменська, арабська, іврит тощо.',
    facts: [
      { label: 'Термін видачі', value: '1–3 дні', note: 'standardowo · терміново — 24 год' },
      { label: 'Вартість', value: '~80 PLN', note: 'за стор. 1125 знаків · з/на польську' },
      { label: 'Мови', value: '35+', note: 'усі мови ЄС + азійські + пострадянських країн' },
      { label: 'Де приймають', value: 'Усі urzędy', note: 'sądy, banks, USC, ZUS, KRS' }
    ],
    grounds: {
      title: 'Які документи перекладаємо',
      subtitle: 'Повний перелік — на сайті TEPIS. Найбільш затребувані в іноземців:',
      items: [
        { title: 'Свідоцтва USC', desc: 'Про народження, шлюб, розлучення, смерть — для umiejscowienia, громадянства, спадщини. Ставка 80 PLN/сторінка.' },
        { title: 'Дипломи та атестати', desc: 'Вища освіта, шкільний атестат, професійні сертифікати — для нострифікації в Польщі.' },
        { title: 'Документи для карти побуту/карти сталого побуту', desc: 'Umowy o pracę, довідки про доходи, виписки рахунків, договори оренди — пакет іноземною мовою перекладаємо під ключ.' },
        { title: 'Водійські права', desc: 'Обмін українських прав на польські — обов’язковий присяжний переклад обох сторін. 80–120 PLN.' },
        { title: 'KRK / довідки про несудимість', desc: 'З країн походження — для Karty pobytu, громадянства, ліцензій.' },
        { title: 'Корпоративні документи', desc: 'KRS-виписки, статути, протоколи зборів, баланси — для реєстрації фірми або угод.' },
        { title: 'Судові рішення', desc: 'Wyroki rozwodowe, alimentacyjne, spadkowe — для виконання в Польщі або umiejscowienia.' },
        { title: 'Медичні документи', desc: 'Історії хвороби, рецепти, довідки про щеплення, донорство органів — для NFZ або роботи.' }
      ]
    },
    timeline: {
      title: 'Як працює присяжний переклад',
      subtitle: 'Від передачі документа до готового перекладу з печаткою — 1–3 робочих дні.',
      steps: [
        { title: 'Передача документа', duration: '5 хв', desc: 'Надсилаєте скан / фото / PDF на email або передаєте оригінал особисто. Розбірливість — критична: якість перекладу залежить від читабельності джерела.' },
        { title: 'Розрахунок вартості', duration: '15 хв', desc: 'Рахуємо кількість стандартних сторінок (1125 знаків з пробілами польською). Виставляємо точний кошторис протягом години.' },
        { title: 'Підписання замовлення', duration: '5 хв', desc: 'Підтверджуєте термін і ціну, сплачуєте 50% авансу або після готовності (постійним клієнтам).' },
        { title: 'Переклад', duration: '1–3 дні', desc: 'Tłumacz przysięgły виконує переклад, засвідчує підписом, печаткою та формулою, що переклад відповідає оригіналу. Кожен аркуш пронумерований.' },
        { title: 'Передача готового перекладу', duration: '15 хв', desc: 'Паперова версія — забираєте в офісі або кур’єром. Електронна — з qualified signature на ePUAP. Для urzędów — обидва варіанти працюють.' },
        { title: 'Apostille (опційно)', duration: '3–5 днів', desc: 'Якщо переклад використовується за кордоном — ставимо apostille MSZ на польський оригінал і присяжний переклад разом.' }
      ]
    },
    documents: {
      title: 'Що потрібно від вас',
      subtitle: 'Мінімум для розрахунку вартості та термінів.',
      items: [
        'Скан або фото документа у високому розширенні (≥ 300 dpi)',
        'Оригінал документа — для звірки або особистої зустрічі',
        'Усі печатки, штампи, підписи повинні бути читабельні',
        'Перелік власних імен у правильній транскрипції (Ваше Ім’я, прізвище)',
        'Apostille або легалізація (якщо вже стоїть — перекладаємо разом з ним)',
        'Мета використання (urząd, sąd, ZUS, USC) — визначає формат',
        'Терміновість — стандарт (1–3 дні) або express (24 год, +50%)',
        'Реквізити отримувача (якщо потрібна електронна версія з QES)'
      ]
    },
    pricing: {
      title: 'Вартість роботи',
      subtitle: 'Ціна за стандартну сторінку (1125 знаків з пробілами польською).',
      items: [
        { label: 'З/на польську (українська, російська)', value: 'від 70 PLN', note: 'За сторінку 1125 знаків' },
        { label: 'З/на англійську, німецьку', value: 'від 80 PLN', note: 'Державний тариф МС' },
        { label: 'Рідкісні мови (грузинська, вірменська)', value: 'від 100 PLN', note: 'Арабська, іврит, фарсі' },
        { label: 'Терміново (24 години)', value: '+50%', note: 'Доступно для більшості мов' }
      ],
      note: 'Зазначено базові ставки за сторінку 1125 знаків. Остаточна ціна залежить від обсягу (у сторінках після перекладу, не в оригіналі), мови та терміновості. Мінімальне замовлення — 1 сторінка. Знижки — для юр. осіб і постійних клієнтів.'
    },
    legalBase: {
      title: 'Нормативна база',
      items: [
        'Ustawa z dnia 25 listopada 2004 r. o zawodzie tłumacza przysięgłego',
        'Rozporządzenie Ministra Sprawiedliwości w sprawie wynagrodzenia za czynności tłumacza przysięgłego (стандартні ставки)',
        'Lista tłumaczy przysięgłych — Ministerstwo Sprawiedliwości (publiczny rejestr)',
        'Rozporządzenie (UE) 2016/1191 (eliminacja legalizacji w UE — також спрощує вимоги до перекладів)',
        'Ustawa z dnia 14 czerwca 1960 r. — Kodeks postępowania administracyjnego (art. 5 — wymóg języka polskiego)'
      ]
    },
    faq: [
      { q: 'Чим присяжний переклад відрізняється від звичайного?', a: 'Присяжний переклад має юридичну силу — прирівняний до нотаріального засвідчення. Tłumacz przysięgły — державний екзаменований спеціаліст у реєстрі MS, його переклад приймають urzędy, sądy, банки, USC. Звичайний перекладач — навіть ідеальний — не має такого статусу. Для офіційних документів потрібен лише присяжний.' },
      { q: 'Скільки коштує переклад одного свідоцтва?', a: 'Свідоцтво про народження/шлюб (1 сторінка) — 70–80 PLN. Диплом + додаток (3–5 сторінок) — 250–400 PLN. Umowa o pracę (4–8 сторінок) — 320–650 PLN. Точна ціна — після оцінки протягом години.' },
      { q: 'Чи можна отримати переклад в електронному вигляді?', a: 'Так. З 2018 року tłumacz przysięgły може засвідчувати переклади qualified electronic signature (QES). Електронна версія має ту саму юридичну силу, що й паперова. Для подання через ePUAP / MOS v2.0 / e-Urząd Skarbowy — це ідеальний формат.' },
      { q: 'Скільки діє присяжний переклад?', a: 'Сам переклад — безстроково. Але багато документів мають свій термін: KRK — 6 міс., KRS-виписка — 3 міс. Робіть переклад одразу після отримання свіжого оригіналу — інакше документ застаріє до подання.' },
      { q: 'Чи потрібен apostille перед перекладом?', a: 'Залежить від мети. Для використання в Польщі — apostille не потрібен (для документів з країн Конвенції 1961 р.). Для використання за кордоном — apostille ставиться на оригінал, потім перекладаємо оригінал і apostille разом. Іноді процедура вимагає двох apostille — один на оригінал, другий на переклад. Уточнюємо у цільової установи.' },
      { q: 'Що якщо документ написаний від руки або погано читається?', a: 'Tłumacz przysięgły може відмовитися або зробити переклад з позначкою «nieczytelne» в місцях, де не вдається розібрати. Це знижує шанси прийняття документа. Робіть якісний скан (≥ 300 dpi) при гарному освітленні.' },
      { q: 'Чи можна засвідчити переклад, зроблений мною?', a: 'Ні. Присяжний перекладач засвідчує лише переклад, виконаний особисто або під його контролем. Це правило суворе — порушення тягне за собою позбавлення ліцензії. Якщо у вас вже є переклад — він буде прийнятий як «звірка», але це та сама вартість.' }
    ],
    related: ['apostille', 'akty-stanu-cywilnego', 'pesel'],
    ctaTitle: 'Присяжний переклад за 1–3 дні',
    ctaSubtitle: 'Безкоштовна оцінка: надішліть скан документа — порахуємо вартість і термін протягом години. Від передачі до готового перекладу з печаткою — 1–3 робочих дні.'
  }
};
