...
const obywatelstwo: ServiceEntry = {
  ru: {
    ...
    timeline: {
      ...
      steps: [
        { title: 'Оценка пути', duration: '1 час', desc: 'Определяем наилучший путь: подтверждение, признание или натурализация.' },
  ...
  uk: {
    ...
    group: 'immigration',
    icon: 'landmark',
    badge: 'Повне право ЄС',
    title: 'Громадянство Польщі',
    subtitle: 'Obywatelstwo Polskie — паспорт ЄС',
    lead: 'Є три шляхи до польського паспорта: потвердження походження (potwierdzenie — якщо мали польське коріння), визнання (uznanie) — після 10 років проживання, і натуралізація (nadanie) — рішенням Президента Польщі.',
    facts: [
      { label: 'Терміну очікування немає', value: 'до 24 міс.', note: 'залежить від шляху' },
      { label: 'Подвійне громадянство', value: 'дозволено', note: 'з боку Польщі' },
      { label: 'Паспорт', value: 'ЄС', note: 'свобода пересування в Шенгені' },
      { label: 'Політичні права', value: 'повні', note: 'включно з голосуванням' }
    ],
    grounds: {
      title: 'Три шляхи до громадянства',
      items: [
        { title: 'Потвердження (potwierdzenie)', desc: 'Якщо ви або ваш предок — громадянин Польщі за народженням і громадянство не було втрачене. Рішення виносить воєвода.' },
        { title: 'Визнання (uznanie)', desc: 'Після 10 років безперервного перебування в Польщі + карта сталого побуту + стабільний дохід + польська B1.' },
        { title: 'Натуралізація (nadanie)', desc: 'За рішенням Президента Польщі. Підстави: 3 роки сталого побуту + інтеграція. Без обов’язкового терміну розгляду.' },
        { title: 'Спрощена натуралізація', desc: 'Для біженців (після 2 років сталого побуту), апатридів, подружжя громадян Польщі (після 3 років шлюбу і 2 років сталого побуту).' }
      ]
    },
    timeline: {
      title: 'Етапи визнання (uznanie)',
      steps: [
        { title: 'Оцінка шляху', duration: '1 год', desc: 'Визначаємо найкращий шлях: потвердження, визнання чи натуралізація.' },
        { title: 'Потвердження B1', duration: '0–6 міс.', desc: 'Складаємо державний іспит із польської мови. Або підтверджуємо звільнення.' },
        { title: 'Збір документів', duration: '4–8 тижнів', desc: 'Архівні довідки, підтвердження доходу, житла, соціальної інтеграції.' },
        { title: 'Подання воєводі', duration: '1 день', desc: 'Заява через воєводу з opłatą skarbową 219 PLN (визнання) або 20 EUR (натуралізація).' },
        { title: 'Розгляд', duration: '6–18 міс.', desc: 'Воєвода — для визнання. Для натуралізації — канцелярія Президента після воєводи.' },
        { title: 'Присяга та паспорт', duration: '1 міс.', desc: 'Постанова про надання → присяга (ślubowanie) → оформлення dowodu osobistego (внутрішнього посвідчення) і закордонного паспорта.' }
      ]
    },
    documents: {
      title: 'Документи',
      items: [
        'Wniosek o uznanie za obywatela polskiego або o nadanie obywatelstwa',
        'Autobiografia у письмовій формі польською',
        'Сертифікат знання польської (B1 мінімум — egzamin certyfikatowy)',
        'Документи сталого побуту (decyzja про сталий побут)',
        'Довідки про доходи за останні 3 роки (PIT, zaświadczenie ZUS)',
        'Підтвердження житла (акт власності, umowa najmu, zameldowanie)',
        'Довідка про несудимість у Польщі та країні походження',
        'Свідоцтва про народження, шлюб і документи предків — для потвердження походження'
      ]
    },
    pricing: {
      title: 'Вартість роботи',
      items: [
        { label: 'Оцінка шляху', value: 'Безкоштовно', note: 'Первинна консультація' },
        { label: 'Визнання (uznanie)', value: 'від 4 500 PLN', note: 'Під ключ' },
        { label: 'Натуралізація (nadanie)', value: 'від 6 000 PLN', note: 'Канцелярія Президента — окремо' },
        { label: 'Потвердження походження', value: 'від 3 500 PLN', note: '+ архівна робота' }
      ],
      note: 'Opłata skarbowa 219 PLN (uznanie) або 20 EUR (nadanie) сплачується клієнтом.'
    },
    legalBase: {
    ...
    faq: [
      { q: 'Який рівень мови потрібен?', a: 'B1 — підтверджується державним сертифікатом (egzamin certyfikatowy z języka polskiego jako obcego), атестатом польської школи, дипломом польського вишу або іншим документом, що підтверджує рівень.' },
      { q: 'Що якщо мій предок емігрував до 1918 року?', a: 'Польщу як державу було відновлено у 1918 році — предки до цієї дати не були громадянами Польщі, і потвердження не спрацює. Лишається шлях uznanie або nadanie.' },
      { q: 'Чи можу я зберегти українське громадянство?', a: 'З боку Польщі — так, вона визнає подвійне громадянство. З боку України: при набутті іноземного громадянства за українським законодавством передбачено втрату українського, але на практиці примусова процедура майже не застосовується. Перевіряйте окремо за вашою країною.' },
      { q: 'Чи є квоти на громадянство?', a: 'Квот немає. Але натуралізація (рішення Президента) — дискреційне рішення, можлива відмова без пояснення причин.' },
      { q: 'Що з дітьми?', a: 'Діти до 18 років можуть отримати громадянство разом із батьком/матір’ю (nabycie pochodne). З 16 років — за згодою самої дитини.' }
    ],
    related: ['karta-stalego-pobytu', 'karta-polaka', 'apelacje'],
    ctaTitle: 'Який шлях до паспорта підходить саме вам?',
    ctaSubtitle: 'За 30 хвилин розберемо ваше коріння, стаж і документи — і покажемо, що реально.'
  }
};
