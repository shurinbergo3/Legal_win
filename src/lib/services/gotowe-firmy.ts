...
const gotoweFirmy: ServiceEntry = {
  ...
  uk: {
    ...
    group: 'business',
    icon: 'landmark',
    badge: 'Shelf company · запуск за 24 години',
    title: 'Готові компанії (shelf-companies)',
    subtitle: 'Sp. z o.o. з історією і рахунком — старт операцій за 24 години',
    lead: 'Готова компанія (shelf company) — Sp. z o.o., яку ми зареєстрували заздалегідь, яка пройшла KYC у банку, з відкритим рахунком і нульовими боргами. Купуєте частку — і одразу підписуєте контракти, виставляєте інвойси, проходите тендери. Кожна фірма «чиста»: з нульовою діяльністю, без зобов’язань, з пройденим VAT. Ідеальне рішення, коли чекати 7–10 днів не можна.',
    facts: [
      { label: 'Запуск', value: '24 години', note: 'від підписання до операцій' },
      { label: 'Історія фірми', value: '1–24 міс.', note: 'дата в KRS' },
      { label: 'VAT-статус', value: 'активний', note: 'czynny VAT не потрібно чекати' },
      { label: 'Банк-рахунок', value: 'відкритий', note: 'mBank / ING / Santander' }
    ],
    grounds: {
      title: 'Коли потрібна готова Sp. z o.o.',
      subtitle: 'Готова фірма коштує дорожче, але дає швидкість і історію — критично для тендерів, банків і великих клієнтів.',
      items: [
        { title: 'Терміновий контракт', desc: 'Клієнт готовий підписати в середу — а Sp. z o.o. з нуля займе 7–10 днів. Готова — пишете NIP в umowę у день звернення.' },
        { title: 'Тендер із цензом', desc: 'Багато zamówienia publiczne вимагають opyty — фірми віком 6+ або 12+ місяців. Готова фірма з потрібною датою реєстрації проходить ценз.' },
        { title: 'Відкриття рахунку в суворому банку', desc: 'Pekao, Citi, BNP Paribas неохоче відкривають рахунки свіжим Sp. z o.o. без оборотів. Готова фірма вже пройшла KYC — рахунок працює.' },
        { title: 'Кредит або leasing', desc: 'Банки і leasing-компанії не дають фінансування фірмам < 12 місяців. Готова Sp. z o.o. з історією — інша історія.' },
        { title: 'Реєстрація на маркетплейсах', desc: 'Allegro, Empik, Amazon Marketplace вимагають активний VAT і історію. Готова фірма проходить реєстрацію за день.' },
        { title: 'Немає PESEL у засновника', desc: 'Реєстрація Sp. z o.o. з нуля потребує PESEL у członka zarządu (для KRS-online). Готова фірма вже зареєстрована — можна змінити członka zarządu пізніше.' }
      ]
    },
    timeline: {
      title: 'Як купуєте готову фірму',
      subtitle: 'Від першого дзвінка до першої фактури — зазвичай 1–2 робочих дні.',
      steps: [
        { title: 'Каталог', duration: '5 хвилин', desc: 'Показуємо доступні фірми: дата реєстрації, PKD, статус VAT, банк, адреса. Підбираємо під вашу нішу.' },
        { title: 'Резервування', duration: 'на 24 години', desc: 'Бронюємо фірму — нікому не запропонуємо. KYC покупця: passport, джерело коштів, бенефіціар.' },
        { title: 'Договір купівлі udziałów', duration: '1 день', desc: 'Umowa zbycia udziałów у нотаріуса у Варшаві. Якщо ви в іншому місті — pełnomocnictwo через консула.' },
        { title: 'Зміна członka zarządu', duration: '1 день', desc: 'Призначаємо нового члена правління, попередній виходить. Wniosek у KRS.' },
        { title: 'Передача рахунку', duration: '1–2 дні', desc: 'Відкликання повноважень у попередніх підписантів у банку, додавання нових. Можна зберегти старий рахунок або відкрити новий.' },
        { title: 'CRBR оновлення', duration: '7 днів', desc: 'Реєстрація нового бенефіціара в Centralnym Rejestrze. Робимо за вас.' },
        { title: 'Оновлення KRS', duration: '7–14 днів', desc: 'KRS відображає зміни. Але NIP, VAT, банк працюють одразу — виписки KRS актуалізуються автоматично.' }
      ]
    },
    documents: {
      title: 'Що потрібно від покупця',
      subtitle: 'Мінімум документів — KYC і підписи.',
      items: [
        'Паспорт нового wspólnika і członka zarządu',
        'PESEL члена правління (або оформлюємо за 7–14 днів)',
        'Адреса реєстрації членів правління (для CRBR)',
        'Підтвердження джерела коштів для KYC (для банку)',
        'Pełnomocnictwo notarialne — якщо не приїдете (apostille в країні проживання)',
        'Згода на зміну członka zarządu (від попереднього члена — оформлюємо ми)',
        'NIP і REGON — вже є у фірми'
      ]
    },
    pricing: {
      title: 'Вартість готових фірм',
      subtitle: 'Ціна залежить від віку, статусу VAT, банку, історії. Держмита KRS, нотаріус — окремо.',
      items: [
        { label: 'Фірма 1–3 міс.', value: 'від 6 500 PLN', note: 'Базова: KRS + NIP + банк' },
        { label: 'Фірма 6–12 міс.', value: 'від 9 800 PLN', note: 'Активний VAT, історія' },
        { label: 'Фірма 12+ міс.', value: 'від 14 500 PLN', note: 'Для тендерів і кредитів' },
        { label: 'Преміум — банк A/A+', value: 'від 18 000 PLN', note: 'Pekao, Citi, BNP — рахунок' }
      ],
      note: 'Ціна включає наш гонорар і капітал фірми (5 000 PLN). Держмита KRS 250 PLN, нотаріус 1 200–2 500 PLN, CRBR — у нас у вартості. Термінове оформлення за 24 години — +30%.'
    },
    legalBase: {
    ...
    faq: [
      { q: 'Чи безпечно купувати готову Sp. z o.o.?', a: 'Якщо фірма від надійного провайдера — так. Усі наші фірми: нуль операцій (лише реєстрація), нуль зобов’язань, чиста бухгалтерія, актуальне sprawozdanie finansowe (фінансовий звіт). Перед продажем проводимо due diligence: KRS (Державний судовий реєстр), ZUS, US (податкова), KRD, sprawozdania (звіти) за всі періоди. Документи передаємо разом із фірмою.' },
      { q: 'Що з минулими зобов’язаннями?', a: 'Готова фірма не мала жодної діяльності — немає договорів, контрагентів, кредитів, ZUS-боргів. Додатково даємо гарантію: якщо спливе щось «минуле» — компенсуємо збитки. У договорі прописано.' },
      { q: 'Чи можна змінити назву і PKD?', a: 'Так. Через uchwałę wspólników (рішення учасників) змінюємо nazwę (назву), PKD, siedzibę (юридичну адресу), kapitał (капітал). Потребує походу до нотаріуса або подання через KRS (судовий реєстр). Робимо за 7–14 днів. Вартість 700–1 500 PLN.' },
      { q: 'Чи зберігається VAT-номер при зміні власника?', a: 'Так. NIP і VAT залишаються у фірми (це атрибути юридичної особи, а не засновника). Головне — після зміни członka zarządu (члена правління) оновити дані в Urzędzie Skarbowym (податковій) через VAT-Z.' },
      { q: 'Що з банківським рахунком?', a: 'Залежить від банку. mBank, ING, Santander зазвичай зберігають рахунок після зміни підписантів (новий KYC). Pekao, Citi можуть вимагати нову реєстрацію рахунку. Перевіряємо перед продажем і попереджаємо.' },
      { q: 'Скільки років фірма має бути «активною»?', a: 'Залежить від мети. Для маркетплейсів — достатньо 1+ місяця. Для тендерів — зазвичай 12+. Для великих кредитів — 24+. На сайті вказано, під які цілі підходить кожна фірма.' },
      { q: 'А чи можна купити з пройденим оборотом і податковими звітами?', a: 'Тільки наші фірми — з нульовою діяльністю, тому що купівля фірми з реальною історією — високий ризик (приховані зобов’язання, податкові спори). Якщо потрібна реальна історія, рекомендуємо зареєструвати Sp. z o.o. під себе і працювати активно — це безпечніше довгостроково.' }
    ],
    related: ['rejestracja-sp-z-oo', 'jdg', 'podatki-ksiegowosc'],
    ctaTitle: 'Готова Sp. z o.o. за 24 години',
    ctaSubtitle: 'Каталог з 30+ фірм з різною історією і банками. Безкоштовно підбираємо під ваш бізнес і цілі.'
  }
};
