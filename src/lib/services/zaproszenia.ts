...
const zaproszenia: ServiceEntry = {
  ...
  uk: {
    ...
    group: 'relocation',
    icon: 'fingerprint',
    badge: 'Zaproszenie · віза D · Schengen',
    title: 'Електронні запрошення',
    subtitle: 'Zaproszenia для шенгенських і національних віз',
    lead: 'Zaproszenie — офіційне запрошення від польської фізичної або юридичної особи, що реєструється в Urzędzie Wojewódzkim. Використовується для оформлення шенгенської візи C або національної D у консульстві Польщі за кордоном, коли в заявника немає запрошення від роботодавця чи туристичної броні. Працюємо як запрошувальна сторона: за 7–14 днів реєструємо zaproszenie, передаємо оригінал кур’єром і супроводжуємо візову процедуру.',
    facts: [
      { label: 'Термін реєстрації', value: '7–14 днів', note: 'wojewoda · standard' },
      { label: 'Держмито', value: '27 PLN', note: 'opłata skarbowa za zaproszenie' },
      { label: 'Діє', value: '12 міс', note: 'з дати внесення в реєстр' },
      { label: 'Де реєструється', value: 'Urząd Wojewódzki', note: 'за місцем проживання запрошувача' }
    ],
    grounds: {
      title: 'Коли потрібне zaproszenie',
      subtitle: 'Без zaproszenia консульство відмовить, якщо в заявника немає іншої підстави (umowa o pracę, gwarancja firmy, бронь готелю).',
      items: [
        { title: 'Віза D для тривалого перебування', desc: 'Національна віза до 365 днів — для возз’єднання сім’ї, догляду за родичем, візиту партнера. Через zaproszenie від резидента Польщі.' },
        { title: 'Віза Schengen C — туристична', desc: 'Короткострокова до 90 днів. Коли в заявника немає броні готелю — родич або друг у Польщі робить zaproszenie.' },
        { title: 'Ділова віза для контрагента', desc: 'Sp. z o.o. або JDG запрошує контрагента з України та інших країн — постачальника, клієнта, ділового партнера. Реєструється на фірму.' },
        { title: 'Навчальний візит / стажування', desc: 'Виш або роботодавець запрошує на стажування до 6 місяців — без окремого zezwolenie na pracę.' },
        { title: 'Сімейний візит', desc: 'Родич у Польщі запрошує батьків, дітей, чоловіка/дружину, бабусю — для тривалого перебування.' },
        { title: 'Відвідування тяжкохворого', desc: 'Терміновий візит до хворого родича — експрес-zaproszenie за 3–5 днів із медичною довідкою.' },
        { title: 'Релігійний або культурний візит', desc: 'Для священнослужителів, артистів, спортсменів — реєстрація через wniosek або wzór C.' },
        { title: 'Весілля в Польщі', desc: 'Наречений/наречена запрошує партнера для реєстрації шлюбу в USC — зазвичай D-віза з zaproszeniem.' }
      ]
    },
    timeline: {
      title: 'Як ми робимо zaproszenie',
      subtitle: 'Від першого дзвінка до отримання візи заявником — зазвичай 4–8 тижнів.',
      steps: [
        { title: 'Консультація', duration: '30 хв', desc: 'Уточнюємо мету візиту, ступінь спорідненості, тип візи (C/D), терміни. Перевіряємо платоспроможність запрошувача.' },
        { title: 'Wniosek o zaproszenie', duration: '1 день', desc: 'Заповнюємо wniosek із додатками: tytuł prawny do mieszkania, виписки з рахунку, PIT-37 за минулий рік, KRS (для фірми).' },
        { title: 'Подання в Urząd Wojewódzki', duration: '1 день', desc: 'Подаємо через MOS v2.0 або фізично. Opłata skarbowa 27 PLN та pełnomocnictwo 17 PLN.' },
        { title: 'Postępowanie wojewody', duration: '7–14 днів', desc: 'Wojewoda перевіряє фінанси запрошувача (мінімум 515 PLN на гостя на місяць + житло).' },
        { title: 'Внесення в реєстр', duration: '1 день', desc: 'Zaproszenie отримує унікальний номер і вноситься в централізований реєстр zaproszeń (видимий консулам).' },
        { title: 'Передача оригіналу', duration: '3–7 днів', desc: 'Надсилаємо оригінал кур’єром DHL / Pocztą Polską до країни заявника. Електронна копія — на email одночасно.' },
        { title: 'Візова заява', duration: '14–30 днів', desc: 'Заявник іде в консульство Польщі з zaproszeniem + анкетою + документами. Консультація щодо списку — у нас.' }
      ]
    },
    documents: {
      title: 'Що потрібно від вас (запрошувача)',
      subtitle: 'Від заявника — окремий пакет у посольстві.',
      items: [
        'Паспорт запрошувача (PESEL обов’язковий)',
        'Документ про право на житло: akt notarialny / договір оренди / zaświadczenie o zameldowaniu',
        'Підтвердження доходу: PIT-37 за минулий рік, ZUS RMUA, виписки з рахунку за 3 міс.',
        'Мінімальна сума — 515 PLN/гість/місяць (200 EUR · потрібно підтвердити)',
        'Зобов’язання покрити витрати на повернення запрошеного',
        'KRS-виписка (якщо запрошує фірма) + статут + KRK członka zarządu',
        'Список запрошених із паспортними даними та ступенем спорідненості',
        'Заповнений wniosek o zaproszenie — готуємо ми'
      ]
    },
    pricing: {
      title: 'Вартість роботи',
      subtitle: 'Фіксована ціна за zaproszenie. Держмита — окремо.',
      items: [
        { label: 'Консультація', value: 'Безкоштовно', note: '30 хвилин · оцінка шансів' },
        { label: 'Zaproszenie під ключ', value: 'від 600 PLN', note: 'Документи, wniosek, urząd, кур’єр' },
        { label: 'Терміново (3–5 днів)', value: 'від 950 PLN', note: 'Express tariff в urzędzie' },
        { label: 'Zaproszenie + супровід візи', value: 'від 1 200 PLN', note: 'З консультацією заявника' }
      ],
      note: 'Зазначено гонорари LegalWin. Держмито 27 PLN, pełnomocnictwo 17 PLN, кур’єр DHL ~120 PLN — окремо.'
    },
    legalBase: {
    ...
    faq: [
      { q: 'Скільки грошей потрібно показати на рахунку?', a: 'Мінімум 515 PLN на кожного гостя на місяць + 200 PLN на дорогу назад. На сім’ю з 3 на місяць — близько 1 850 PLN. Більшість wojewodów вимагають виписки за 3 місяці зі стабільним залишком ≥ суми розрахунку.' },
      { q: 'Чи може zaproszenie зробити Sp. z o.o. без обертів?', a: 'Складно. Wojewoda (воєвода) перевіряє реальність фірми — KRS, оберт за ZUS, наявність bilansu (балансу). Свіжа фірма без deklaracji VAT-7 (декларації ПДВ) і працівників — зазвичай відмова. Краще оформлювати на фізичну особу або через діючу фірму.' },
      { q: 'А якщо родич не в Польщі — чи можна zaproszenie з України?', a: 'Ні. Zaproszenie (запрошення) видає лише польський urząd (установа), і запрошувач повинен мати legalny pobyt w Polsce (легальне перебування в Польщі — PESEL, karta pobytu, громадянство). З України можна оформити лише oświadczenie o zaproszeniu (заяву про запрошення для безкоштовної візи), але це інше.' },
      { q: 'Чи можна запросити без ступеня спорідненості?', a: 'Так — друг, партнер, колега, контрагент. Головне — підтвердити мету візиту (особисте знайомство, ділова зустріч, захід). Без чіткої мети wojewoda може відмовити.' },
      { q: 'Скільки діє zaproszenie?', a: '12 місяців із дати внесення в реєстр. Заявник має подати на візу протягом цього терміну. Сама віза D видається максимум на 365 днів. Schengen C — на той термін, який дозволяє zaproszenie.' },
      { q: 'Що робити, якщо отримали відмову?', a: '14 днів на odwołanie do Szefa UdSC. Найчастіше відмови — за недостатні фінанси або сумніви в меті візиту. Оскарження триває 30–60 днів. Робимо під ключ.' },
      { q: 'Чи дає віза за zaproszeniem право на роботу?', a: 'Ні. Zaproszenie ≠ zezwolenie na pracę. Для роботи потрібен окремий дозвіл від воєводи або Powiatowego Urzędu Pracy. Віза D з zaproszeniem — для перебування, не для роботи.' }
    ],
    related: ['odbior-z-lotniska', 'pomoc-w-zatrudnieniu', 'wyszukiwanie-mieszkania'],
    ctaTitle: 'Zaproszenie за 7–14 днів',
    ctaSubtitle: 'Безкоштовна консультація: перевіримо фінанси та підготуємо пакет. Від підписання договору до zaproszenia в реєстрі — зазвичай 7–14 робочих днів.'
  }
};
