...
const najemKomercyjny: ServiceEntry = {
  ru: {
    ...
    faq: [
      ...
      { q: 'Что делать с lockdownами?', a: 'После Covid в каждой umowie прописываем «klauzulę pandemiczną»: при ограничении usage помещения (lockdown, restrykcje sanitarne) — czynsz обнуляется или уменьшается на 50%. Wynajemcy сопротивляются, но 70% соглашаются после переговоров.' },
      { q: 'Гарантия bankowa или depozyt?', a: 'Bankowa — wynajemcy предпочитают, потому что банк гарантирует выплату. Стоит арендатору 1–2% suma rocznie (provision bankowi). Depozyt — фактически 3–6 мес czynszu lежат на rachunku wynajemcy 5 лет — потеря процентов. Если cashflow важен — bankowa, если есть деньги — depozyt.' },
      { q: 'PCC при umowie najmu?', a: 'PCC 0,5% wartości umowy для umów na czas określony powyżej 1 roku (sumują wszystkie czynsze za okres). Płaci arendатор. Для umów krótszych или nieokreślonych — bez PCC.' }
    ],
