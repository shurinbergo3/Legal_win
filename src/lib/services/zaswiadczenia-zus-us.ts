...
const zaswiadczeniaZusUs: ServiceEntry = {
  ru: {
    ...
    faq: [
      { q: 'Можно ли получить справку без личного визита?', a: 'Да. Большинство справок ZUS — через PUE ZUS (нужен profil zaufany ePUAP). Большинство справок US — через e-Urząd Skarbowy. Бумажная версия — по почте или курьером. Личный визит нужен только для apostille.' },
      { q: 'Как долго действует zaświadczenie o niezaleganiu?', a: 'ZUS — 1 месяц, US — 3 месяца от даты выдачи. Для банков и тендеров обычно требуют свежие справки (≤ 30 дней). Делаем на старте процесса, чтобы успеть с подачей.' },
      { q: 'Что делать, если есть задолженность?', a: 'Нельзя получить «niezaleganie», пока не погасили долг. Помогаем составить wniosek o rozłożenie na raty (рассрочка) или umorzenie (списание) — в исключительных случаях. После погашения — справка выдаётся в день обращения.' },
      ...
      { q: 'Что такое A1 и когда нужен?', a: 'Zaświadczenie A1 — это документ ЕС, подтверждающий, что вы платите соцвзносы только в Польше при работе в другой стране ЕС. Обязательно для дальнобойщиков, IT-фрилансеров с клиентами в ЕС, командировок свыше 24 месяцев. Без A1 принимающая страна может потребовать платить соцвзносы у себя.' },
