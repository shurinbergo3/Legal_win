...
const zaswiadczeniaZusUs: ServiceEntry = {
  ...
  uk: {
    ...
    icon: 'landmark',
    badge: 'PUE ZUS · e-Urząd Skarbowy',
    title: 'Довідки ZUS / US',
    subtitle: 'Zaświadczenia z ZUS і Urzędu Skarbowego',
    lead: 'Zaświadczenia з ZUS (Zakład Ubezpieczeń Społecznych) та Urzędu Skarbowego — ключові документи для подання на карту побуту, карту сталого побуту, іпотеку, кредит, візу за кордон, тендери в публічних закупівлях. Отримуємо довідки про доходи (PIT-11, ZUS RMUA), про відсутність заборгованості (zaświadczenie o niezaleganiu), про страхові періоди (zaświadczenie ZUS S-1) і десятки інших — через PUE ZUS, e-Urząd Skarbowy або фізично в офісі.',
    facts: [
      { label: 'Термін видачі', value: '1–7 днів', note: 'standard ZUS · до 14 днів у складних випадках' },
      { label: 'Держмито', value: '0–21 PLN', note: 'базові — безкоштовно' },
      { label: 'Де', value: 'PUE ZUS + EUS', note: 'онлайн або офіс' },
      { label: 'Діє', value: '1–6 міс', note: 'залежно від типу довідки' }
    ],
    grounds: {
      title: 'Які довідки отримуємо',
      subtitle: 'Повний перелік — на сайті biznes.gov.pl. Найбільш затребувані в іноземців — нижче.',
      items: [
        { title: 'Zaświadczenie o niezaleganiu w ZUS', desc: 'Підтвердження відсутності заборгованості за соцвнесками. Обов’язково для карти побуту за бізнесом, тендерів, кредитів. Термін — 7 днів, безкоштовно.' },
        { title: 'Zaświadczenie o niezaleganiu w US', desc: 'Аналог з Urzędu Skarbowego — немає боргів за податками. 7 днів, 21 PLN. Для тендерів, іпотеки, ліцензій.' },
        { title: 'PIT-11 за минулі роки', desc: 'Дублікат розрахунку податку від роботодавця — для карти побуту, карти сталого побуту, іпотеки. Якщо роботодавець не відповідає — беремо через US.' },
        { title: 'ZUS RMUA — звіт про страхові внески', desc: 'Перелік сплачених внесків і періодів роботи — для підтвердження стажу в Польщі. Безкоштовно через PUE ZUS.' },
        { title: 'Zaświadczenie A1 (відрядження)', desc: 'Для працівників, відряджених за кордон — підтверджує сплату ZUS лише в Польщі. Обов’язково для перевізників і IT-фрілансерів.' },
        { title: 'Zaświadczenie ZUS S-1 / U2', desc: 'Підтвердження страхових періодів для пенсії або виплат у країні ЄС.' },
        { title: 'Історична виписка z konta podatnika', desc: 'Усі платежі, переплати, недоплати податків за обраний період — для аудиту, контролю або звітності.' },
        { title: 'Zaświadczenie o dochodach (US)', desc: 'Підтвердження річного доходу — для іпотеки, шкільних стипендій, MOPS-виплат, посольств.' }
      ]
    },
    timeline: {
      title: 'Як ми отримуємо довідку',
      subtitle: 'Стандартний термін — 1–7 робочих днів через електронні платформи або фізично.',
      steps: [
        { title: 'Консультація', duration: '15 хв', desc: 'Уточнюємо тип довідки, мету використання, формат (електронна чи паперова), термін дії.' },
        { title: 'Довіреність', duration: '1 день', desc: 'Підписуємо pełnomocnictwo (UPL-1 для US, ZUS-PEL для ZUS) — 17 PLN держмита за кожну. Через ePUAP — безкоштовно.' },
        { title: 'Подання wniosku', duration: '1 день', desc: 'Заповнюємо і подаємо через PUE ZUS, e-Urząd Skarbowy або фізично — залежно від довідки.' },
        { title: 'Оплата (якщо потрібна)', duration: '1 день', desc: '21 PLN за zaświadczenie z US, безкоштовно — більшість ZUS-довідок.' },
        { title: 'Видача довідки', duration: '1–7 днів', desc: 'Електронна версія — на профіль PUE/EUS. Паперова — забираємо особисто або приймаємо поштою/кур’єром.' },
        { title: 'Apostille або переклад', duration: '3–7 днів', desc: 'Якщо довідка для використання за кордоном — оформлюємо apostille (60 PLN MSZ) і присяжний переклад.' }
      ]
    },
    documents: {
      title: 'Що потрібно від вас',
      subtitle: 'Мінімум для більшості довідок.',
      items: [
        'Паспорт (копія сторінки з фото)',
        'PESEL або NIP (для бізнесу)',
        'Karta pobytu або інший документ легального перебування',
        'Pełnomocnictwo для LegalWin (UPL-1 для US, ZUS-PEL для ZUS)',
        'KRS-номер фірми (якщо довідка для Sp. z o.o.)',
        'Доступ до ePUAP / profil zaufany (якщо є — пришвидшує процес)',
        'Зазначення цільової установи (банк, urząd, посольство — формат може відрізнятися)',
        'Період, за який потрібна довідка (наприклад, доходи 2023–2024)'
      ]
    },
    pricing: {
      title: 'Вартість роботи',
      subtitle: 'Гонорар за довідку — від 150 PLN. Держмита — окремо.',
      items: [
        { label: 'Консультація', value: 'Безкоштовно', note: '15 хвилин · уточнення мети' },
        { label: 'Одна довідка під ключ', value: 'від 150 PLN', note: 'Wniosek, довіреність, видача' },
        { label: 'Пакет 3+ довідок', value: 'від 350 PLN', note: 'Знижка при одночасному поданні' },
        { label: 'Довідка + apostille + переклад', value: 'від 480 PLN', note: 'Для використання за кордоном' }
      ],
      note: 'Зазначено гонорари LegalWin. Держмита (0–21 PLN на довідку, 17 PLN pełnomocnictwo, 60 PLN apostille) і присяжний переклад (~80 PLN/документ) — окремо.'
    },
    legalBase: {
    ...
    faq: [
      { q: 'Чи можна отримати довідку без особистого візиту?', a: 'Так. Більшість довідок ZUS — через PUE ZUS (Платформа електронних послуг ZUS, потрібен profil zaufany ePUAP — довірений профіль). Більшість довідок US (Urząd Skarbowy — податкова) — через e-Urząd Skarbowy (електронна податкова). Паперова версія — поштою або кур’єром. Особистий візит потрібен лише для apostille.' },
      { q: 'Як довго діє zaświadczenie o niezaleganiu?', a: 'Zaświadczenie o niezaleganiu (довідка про відсутність заборгованості) — ZUS — 1 місяць, US — 3 місяці від дати видачі. Для банків і тендерів зазвичай вимагають свіжі довідки (≤ 30 днів). Робимо на старті процесу, щоб встигнути з поданням.' },
      { q: 'Що робити, якщо є заборгованість?', a: 'Не можна отримати «niezaleganie» (довідку про відсутність заборгованості), поки не погасили борг. Допомагаємо скласти wniosek o rozłożenie na raty (заяву про розстрочку) або umorzenie (списання) — у виняткових випадках. Після погашення — довідка видається в день звернення.' },
      { q: 'Чим відрізняється ZUS RMUA від PIT-11?', a: 'PIT-11 — розрахунок податку від роботодавця за календарний рік. RMUA — виписка внесків ZUS, які роботодавець зробив за вас. PIT-11 потрібен для карти побуту та податкової декларації. RMUA — для підтвердження стажу.' },
      { q: 'Що таке A1 і коли потрібен?', a: 'Zaświadczenie A1 (свідоцтво A1) — це документ ЄС, що підтверджує: ви сплачуєте соцвнески лише в Польщі при роботі в іншій країні ЄС. Обов’язково для далекобійників, IT-фрілансерів з клієнтами в ЄС, відряджень понад 24 місяці. Без A1 приймаюча країна може вимагати сплачувати соцвнески у себе.' },
      { q: 'Як отримати довідку для іноземного банку / посольства?', a: 'Готуємо довідку, ставимо apostille (якщо країна поза ЄС) і робимо присяжний переклад потрібною мовою. Повний цикл — 7–14 днів. Поширені цілі: іпотека в Німеччині, віза США (DS-160), резидентство в ОАЕ.' },
      { q: 'Скільки фактично коштує отримання всіх довідок для карти побуту?', a: 'Базовий пакет для карти побуту за роботою: PIT-11 (безкоштовно), ZUS RMUA (безкоштовно), zaświadczenie o niezaleganiu (21 PLN US + 0 ZUS). Наш гонорар — 350 PLN. Разом 371 PLN за пакет. З apostille і перекладом — 700–800 PLN.' }
    ],
    related: ['zaswiadczenie-o-niekaralnosci', 'apostille', 'tlumaczenia-przysiegle'],
    ctaTitle: 'Довідки ZUS / US за 1–7 днів',
    ctaSubtitle: 'Безкоштовна консультація: уточнимо тип довідки, мету використання, термін. Від підписання довіреності до отримання — зазвичай 1–7 робочих днів.'
  }
};
