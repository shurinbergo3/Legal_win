...
const wymianaPrawaJazdy: ServiceEntry = {
  ru: {
    ...
    faq: [
      { q: 'Сколько по факту времени занимает обмен прав?', a: 'Стандарт — 30–45 дней от первого визита до польских прав. С теорией WORD — до 60 дней. Срочные случаи (если права истекают) — можем подать без перевода в день консультации, перевод доделаем параллельно.' },
      { q: 'Можно ли водить в Польше с украинскими/белорусскими правами?', a: 'Да, в течение 185 дней проживания. После — обязательно обменять. С 2024 года ITD активно проверяет фактический срок пребывания (PESEL, meldunek, виза). Штраф за управление с просроченным правом обмена — 500 PLN + риск аннулирования.' },
      { q: 'Категории C/D — нужна ли теория?', a: 'Почти всегда — да. Большинство воеводств требуют egzamin teoretyczny WORD для коммерческих категорий из стран не-ЕС. Готовим к экзамену по польским ПДД, выбираем WORD с экзаменом на украинском или английском (зависит от воеводства).' },
      ...
      { q: 'Куда уходят оригиналы иностранных прав?', a: 'Starostwo отправляет оригинал в выдавшую страну (через MSZ или посольство). Это стандартная процедура для предотвращения «двойного владения». Получить их обратно нельзя.' },
      { q: 'Можно ли получить права на катего C/D, если не было такой в стране?', a: 'Нет — обмен возможен только в эквивалентных категориях. Если хотите C/D с нуля — нужен полный курс OSK + Kod 95 (kwalifikacja zawodowa). См. услугу Kod 95.' },
      { q: 'Какие штрафы за управление без замены прав?', a: 'Управление авто с правами, подлежащими обмену, после 185 дней — нарушение art. 94 ustawy Prawo o ruchu drogowym. Штраф 500 PLN + 6 пунктов карных. Полиция проверяет дату въезда по PESEL/meldunkу — обмануть нельзя.' }
    ],
