---
slug: shtrafy-itd-bag-12000-pln-vodiel
title: "Kary BAG i ITD 2026: kierowca i przewoźnik mogą stracić do 12 000 PLN za jedno naruszenie"
description: "W 2026 ITD i niemiecki BAG stosują bezprecedensową kontrolę czasu pracy i tachografów. Zatrzymanie prawa jazdy na 3 miesiące na drodze, kary do 12 000 PLN dla przewoźnika za cudzą kartę kierowcy, brak Kodu 95 lub naruszenie odpoczynku."
locale: pl
publishDate: 2026-04-13
author: LegalWin
category: Transport
keywords:
  - kary BAG ITD kierowca
  - czas pracy kierowcy 2026
  - karta tachografu kara
  - Kod 95 kontrola drogowa
  - kary firmy transportowej
relatedServices:
  - /pl/uslugi/kod-95
  - /pl/uslugi/apelacje
schema: Article
---

# Kary BAG i ITD 2026: kierowca i przewoźnik mogą stracić do 12 000 PLN za jedno naruszenie

W skrócie: w 2026 polska Inspekcja Transportu Drogowego (ITD) i niemiecki Bundesamt für Logistik und Mobilität (BAG) prowadzą bezprecedensową kontrolę czasu pracy i tachografów. Najważniejsze nowe sankcje: **natychmiastowe zatrzymanie prawa jazdy na 3 miesiące na drodze** za poważne naruszenia; **kary do 12 000 PLN dla przewoźnika** za jeden epizod (cudza karta tachografu, magnesy, brak Kodu 95). Zmienia to ekonomię małych i średnich firm - jedno naruszenie może zamknąć biznes.

## Co się zmieniło w 2026

EC Mobility Package III (czerwiec 2025) zaostrzył klasyfikację naruszeń. Dyrektywa wymaga od państw członkowskich:

- klasyfikować naruszenia jako „najpoważniejsze" (NN);
- natychmiast zatrzymywać prawa jazdy kat. C/CE na drodze za NN;
- zwiększyć kary dla przewoźników o 100-300 %;
- przekazywać dane NN do unijnego rejestru ERRU.

Polska i Niemcy - najaktywniejsi egzekutorzy 2026.

## Najpoważniejsze naruszenia (NN)

Aktualizacja 2024 w załączniku III dyrektywy 2006/22/WE:

| Naruszenie | Kategoria | Kara dla firmy | Kara kierowcy | Zatrzymanie prawa |
|---|---|---|---|---|
| Cudza karta tachografu | NN | 12 000 PLN | 2 000 PLN | 3 miesiące |
| Magnesy / zakłócenia tachografu | NN | 10 000 PLN | 2 000 PLN | 3 miesiące |
| Brak ważnego Kodu 95 (C/CE) | NN | 8 000 PLN | 2 000 PLN | Do wyjaśnienia |
| Czas jazdy 9h przekroczony o 50%+ | NN | 4 000 PLN | 1 500 PLN | Do 1 mies. |
| Odpoczynek dzienny skrócony o 50%+ | NN | 4 000 PLN | 1 500 PLN | Do 1 mies. |
| Czas pracy tygodniowy +25%+ | NN | 3 000 PLN | 1 000 PLN | Brak |
| Jazda z wygasłym badaniem lekarskim | NN | 2 000 PLN | 800 PLN | Brak |
| Manipulacja danymi tachografu | NN | do 30 000 PLN | do 5 000 PLN | 3 mies. + ściganie karne |

„Bardzo poważne" (BP) - tabele 2-8× niższe.

## Zatrzymanie prawa jazdy na 3 miesiące

Nowa praktyka 2026. Funkcjonariusz / inspektor ITD potwierdzający NN:

1. sporządza protokół;
2. **fizycznie zabiera plastik prawa jazdy**;
3. przekazuje sprawę do WORD o wydanie decyzji o czasowym pozbawieniu uprawnień;
4. WORD wydaje decyzję w 14 dni, zwykle potwierdzającą 3-miesięczne zatrzymanie.

Dla kierowcy:
- 3 miesiące bez prawa = 3 miesiące bez pracy (międzynarodowiec);
- odprawa od pracodawcy może być sporna;
- okres nie wlicza się w cenzus karty pobytu (gdy praca była podstawą).

Dla przewoźnika:
- pilne szukanie zastępcy;
- kara dodatkowa;
- ryzyko utraty wpisu do rejestru przewoźników drogowych przy powtórzonych epizodach w 12 miesiącach.

## Co kontroluje inspektor

Standardowa kontrola ITD (ok. 30 minut):

**1. Dokumenty kierowcy:** prawo jazdy, karta kierowcy do tachografu, świadectwo Kod 95, ADR (jeśli dotyczy), karta pobytu (cudzoziemiec).

**2. Dokumenty pojazdu:** dowód rejestracyjny, polisa OC, świadectwo legalizacji tachografu, badanie techniczne.

**3. Tachograf:** dane z 28 dni + bieżący kurs. Kontrole:
- prawidłowa karta (nie cudza);
- zgodność z faktycznym kursem;
- magnesy / podkładki (wzrokowo);
- pauzy 45-minutowe po 4,5h jazdy;
- odpoczynek dzienny min 11h;
- odpoczynek tygodniowy min 45h.

**4. CMR:** ładunek, faktury, wyciągi.

**5. Karta tachografu kierowcy:** ważność, brak uszkodzeń, tożsamość.

**6. ADR przy materiałach niebezpiecznych:** świadectwo, sprzęt, opakowanie.

## Ochrona kierowcy i firmy

**Poziom kierowcy:**
- karta tachografu - tylko własna; cudza = NN;
- bez magnesów, podkładek, fałszerstw - odpowiedzialność karna;
- znajomość czasu pracy; szkolenie okresowe (Kod 95);
- karta aktualna i nieuszkodzona;
- oryginały w kabinie, kopie w chmurze.

**Poziom firmy:**
- **System compliance** - Tacholiga, Webfleet, Kogena;
- **Regularne pobierania tachografu** - co 28 dni z mocy prawa, w praktyce co tydzień;
- **Audyty wewnętrzne** - kwartalne wyrywkowe próbki;
- **Umowy z klauzulą odpowiedzialności** - kierowca odpowiada za naruszenia, regres firmy (z zastrzeżeniami);
- **Prawnik on-call** - szybka reakcja na kontrolę.

## Co robić przy zatrzymaniu

**Na miejscu:**
1. Bez polemik. Tylko żądane dokumenty.
2. Bez wyjaśnień ustnych. Wszystko w protokole pisemnie.
3. Brak czegoś - „sprawdzę i prześlę w 7 dni".
4. Kopia protokołu kontrolnego.
5. Telefon do dyspozytora / biura.

**Pierwsze 7 dni:**
1. Analiza protokołu z prawnikiem.
2. Zastrzeżenia do protokołu - 7 dni.
3. Zatrzymanie prawa - pilny wniosek do WORD o wstrzymanie wykonania.

**Pierwsze 14 dni:**
1. Odwołanie od kary.
2. Odwołanie od decyzji WORD.

## Obrona przez odwołania

Wiele kar ITD skutecznie kwestionowanych. Częste podstawy:

- **Wady proceduralne protokołu** - brak podpisu, świadków, konkretnego naruszenia;
- **Wątpliwości zapisu tachografu** - zakłócenie zasilania, wymiana karty w serwisie;
- **Błędna klasyfikacja** - NN tam, gdzie powinno być BP;
- **Siła wyższa** - wypadek, choroba, awaria.

W mazowieckim 2024-2025 ok. 35 % odwołań kierowców i firm wygrywa w całości lub części. Z prawnikiem - wyżej.

Stawka LegalWin za reprezentację w sprawie kar ITD/BAG - od 2 800 PLN. Skarga do WSA przy odmowie odwołania - od 1 800 PLN.

## Kiedy potrzebny prawnik

Dla kierowców-najemnych samodzielnie radzą sobie drobne epizody (wygasłe badania, drobne odpoczynki).

Prawnik potrzebny przy:

- mandacie ITD na drodze;
- zatrzymaniu prawa jazdy;
- nakazie zapłaty od firmy;
- kontroli BAG w Niemczech z dużą kwotą;
- ryzyku blokady wpisu do rejestru (firma);
- równoległym roszczeniu pracodawcy o regres.

Dla firm - rekomendowana subskrypcja legal advisory: monitorowanie ryzyk, gotowość do kontroli, higiena compliance. Stawka LegalWin dla firm transportowych - od 2 500 PLN/m-c z szybką reakcją.

[Umów konsultację transportową →](/pl/uslugi/kod-95)

---

Artykuł ma charakter informacyjny. Konkretne ryzyka i obrona zależą od typu przewozów, wielkości firmy, historii naruszeń. W indywidualnych sprawach prosimy o konsultację z prawnikiem.

**Powiązane materiały:**
- [Kod 95 w Polsce: krok po kroku](/pl/blog/kod-95-polsha-kak-poluchit)
- [Kwalifikacja Wstępna vs Szkolenie Okresowe](/pl/blog/kod-95-kwalifikacja-wstepna-vs-szkolenie-okresowe)
- Usługa: [Kod 95 i ochrona kierowcy](/pl/uslugi/kod-95)
